...
//template <class PGraph> void GetBFSGraph(const int& StartNode, const bool& FollowOut, const bool& FollowIn, const int& TargetNId, const int& MxDist);
...
public:
  ...
  int CustomizedBfs(const int& StartNode, const bool& FollowOut, const bool& FollowIn, const int& TargetNId=-1, const int& MxDist=TInt::Mx, const double& sampleRate=1);
...
template<class PGraph>
int TBreathFS<PGraph>::CustomizedBfs(const int& StartNode, const bool& FollowOut, const bool& FollowIn, const int& TargetNId, const int& MxDist, const double& sampleRate) {
  StartNId = StartNode;
  int edges = Graph->GetEdges();
  BFSGraph = TNGraph::New();
  ...
  while (! Queue.Empty()) {
    ...
    BFSGraph->AddNode(NId);

    if (FollowOut) { // out-links
      for (v = 0; v < NodeI.GetOutDeg(); v++) {  // out-links
        const int DstNId = NodeI.GetOutNId(v);
        BFSGraph->AddNode(DstNId);
        BFSGraph->AddEdge(NId, DstNId);
        //printf ("%d, %d \n", NId, DstNId);
        if (! NIdDistH.IsKey(DstNId)) {
          NIdDistH.AddDat(DstNId, Dist+1);
          MaxDist = TMath::Mx(MaxDist, Dist+1);
          ...
          Queue.Push(DstNId);
          //********Stopping Criteria*******
        if (BFSGraph->GetEdges() >= edges * sampleRate )
            return MaxDist;
        }
      }
    }
...
/*template<class PGraph>
void TBreathFS<PGraph>::GetBFSGraph(const int& StartNode, const bool& FollowOut, const bool& FollowIn, const int& TargetNId, const int& MxDist) {
  StartNId = StartNode;
  IAssert(Graph->IsNode(StartNId));
  NIdDistH.Clr(false);
  NIdDistH.AddDat(StartNId, 0);
