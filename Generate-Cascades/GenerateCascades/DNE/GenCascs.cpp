...
void GenCascs::SaveCascades(const TStr& OutFNm) {//*********(CHANGED BY P. SIYARI)********
	TFOut FOut(OutFNm);
	...
	for (int i = 0; i < CascV.Len(); i++) {
		...
		if (C.Len() >= 1){
            //FOut.PutStr(TStr::Fmt(";;;%f(With Log)",C.GetProb(this->GroundTruth)));
            //FOut.PutStr(TStr::Fmt(";%f",C.GetProb(this->GroundTruth)));
            TStr tmp=C.GetProb(this->GroundTruth);
//            cout << "OK" << endl;
            FOut.PutStr(TStr::Fmt("%s",tmp.GetCStr()));
			FOut.PutStr(TStr::Fmt("\r\n"));
        }
	}
}
